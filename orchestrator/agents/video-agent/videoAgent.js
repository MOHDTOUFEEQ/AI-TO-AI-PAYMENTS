  ...
  `;
	...
	try {
		...
		for (let attempt = 1; attempt <= 150; attempt++) {
			const statusRes = await axios.get("https://api.kie.ai/api/v1/veo/record-info", {
				params: { taskId },
			...
			videoUrl = record?.response?.resultUrls?.[0] || record?.response?.originUrls?.[0];
			...
			console.log(`⌛ [${attempt}] Still processing... statusFlag=${flag} (checking again in 2s)`);
